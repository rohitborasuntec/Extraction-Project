...
def main():
    ...
    print(f"[Fetch] Downloaded {total_raw_lines} raw lines")

    all_rows: List[Dict[str, str]] = []
    parsed_lines = 0
    blank_lines = 0
    skipped_no_name = 0
    skipped_examples = []  # (line_number, raw_line) for the debug report
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if not rows:
            skipped_no_name += 1
            skipped_examples.append((line_number, line))
            continue
        all_rows.extend(rows)

    print(f"[Process] {total_raw_lines} raw lines -> {blank_lines} blank, "
          f"{parsed_lines} parsed, {skipped_no_name} skipped (no name) "
          f"-> {len(all_rows)} output rows")

    # Write a debug report of every skipped line so a discrepancy in row
    # counts can be diagnosed precisely instead of guessed at.
    if skipped_examples:
        debug_filename = f"skipped_lines_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(debug_filename, "w", encoding="utf-8") as dbg:
            for line_number, line in skipped_examples:
                dbg.write(f"line {line_number}: {line}\n")
        print(f"[Debug] Wrote {len(skipped_examples)} skipped lines to {debug_filename} for inspection")
